...
class AnswerController extends Controller
{
    public function store(Request $request): RedirectResponse|array
    {
        if (Auth::check() && Auth::user()->role === 'admin') {
            ...
            try {
                ...
                $this->store_model(Auth::user()->id, $enquiry->id, $enquiry->user->id, $enquiry->offer_id, $request->input('message'));
    ...
    private function store_model(int $user_id, int $enquiry_id, int $client_id, int $offer_id, string $text): void
    {
        $answer = new ModelsAnswer();
        $answer->fill(['user_id' => $user_id, 'enquiry_id' => $enquiry_id, 'client_id' => $client_id, 'offer_id' => $offer_id]);
        $answer->message = $text;
        $answer->save();
    }
